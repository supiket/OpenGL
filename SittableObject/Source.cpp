...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "piano chair", NULL, NULL);
    if (window == NULL)
    {
    ...
    Shader simpleShader("simpleShader.vertex", "simpleShader.fragment");
    ...
    unsigned char* data = stbi_load("C:\\Users\\b-oza\\source\\repos\\OpenGL\\x64\\Debug\\leather_baseColor.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 5; i++)
        {
            ...
            if (i > 0) {
                glBindTexture(GL_TEXTURE_2D, footTexture);
                model = glm::scale(model, glm::vec3(0.08f, 0.8f, 0.08f));
            }
            else {
                glBindTexture(GL_TEXTURE_2D, seatTexture);
